...
%%
% **In this simulation we want to  achieve formation dynamic with change position of leader  in AFP path planning with PID controller** % 
%Variables
global  tstep vmax omegamax vmax_leader  sumerrorpi_pos sumerrorpi_theta Perror Perror_dot Thetaerror Thetaerror_dot
%Number of agents
N = 3; 
vmax = 0.5;     %m/s
omegamax = 2;   %rad/s
...
Thetaerror = zeros(2, N);
Thetaerror_dot = zeros(2, N);
...
function [U, W] = controller(P, Theta, A, Pstar, Thetastar, kp, ki, kd)

    global vmax sumerrorpi_pos  sumerrorpi_theta Perror Perror_dot Thetaerror Thetaerorr_dot tstep
    ...
    for i = 1:N
        ...
        Perror_dot(:, i) = (Pstar(:, i) - P(:, i) - Perror(:, i)) / tstep;  %D Controller
        sumerrorpi_pos(:, i) = sumerrorpi_pos(:, i) + tstep * (Pstar(:, i) - P(:, i));  %I Controller
        U(:, i) = kp * (Pstar(:, i) - P(:, i)) + ki * sumerrorpi_pos(:, i) + kd * Perror_dot(:, i);
        sumerrorpi_theta(:, i) = sumerrorpi_theta(:, i) + tstep * (Thetastar(:, i) - Theta(:, i)); %I Controller
        Thetaerorr_dot(:, i) = (Thetastar(:, i) - Theta(:, i) - Thetaerror(:, i)) / tstep;
        W(:, i) = kp * (Thetastar(:, i) - Theta(:, i)) + ki * sumerrorpi_theta(:, i) + kd * Thetaerorr_dot(:, i);
